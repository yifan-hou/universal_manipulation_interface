...
class DiffusionUnetTimmMod1Policy(BaseImagePolicy):
    def __init__(self, 
        ...
        action_dim = action_shape[0] # assuming sparse and dense actions have the same shape
        ...
        # input_dim = time_embeding_dim + dense_obs_dim + cond_dim
        # where:
        #  time_embeding_dim = diffusion_step_embed_dim
        ...
        dense_input_dim = 0
        dense_input_dim += diffusion_step_embed_dim
        for key, attr in shape_meta['sample']['obs']['dense'].items():
            horizon = attr['horizon']
            assert(len(shape_meta['obs'][key]['shape']) == 1) # assuming dense obs shape is 1D
        ...
        dense_output_dim = dense_action_horizon * action_dim
        model_dense = MLP( 
            in_channels = dense_input_dim,
            out_channels = dense_output_dim, # (B, T*D)
            action_horizon = sparse_action_horizon,
            diffusion_step_embed_dim=diffusion_step_embed_dim, # used for time ecoding
        )
    ...
    # ========= training  ============
    def set_normalizer(self, sparse_normalizer: LinearNormalizer, dense_normalizer: LinearNormalizer):
    ...
    def predict_dense_action(self,
        ...
        sparse_ntraj: torch.Tensor, # (B, T, D)
        unnormalize_result: bool = True
    ) -> Dict[str, torch.Tensor]:
        """
        dense_obs_dict: include keys from shape_meta['obs']['dense']. 
            Each key is a tensor of shape (B, H, T, D) or (B, H, T, D1, D2), H might = 1
        ...
        for i in range(X.shape[1]):
            ...
            t = torch.tensor([i / X.shape[1] * self.sparse_action_horizon], dtype=torch.long, device=x.device)
            pred = self.model_dense(x, cond, t) # (B, T*D)
            pred = rearrange(pred, 'b (t d) -> b t d', t=self.dense_action_horizon)
            dense_predictions.append(pred)
        dense_predictions = torch.stack(dense_predictions, dim=1) # (B, H, T, D)
        if unnormalize_result:
            dense_predictions = self.dense_normalizer['action'].unnormalize(dense_predictions)
    ...
    def predict_action(self,
        ...
        cond_data = torch.zeros(size=(batch_size, self.sparse_action_horizon, self.action_dim), device=self.device, dtype=self.dtype)
        ...
        assert sparse_naction_pred.shape == (batch_size, self.sparse_action_horizon, self.action_dim)
        sparse_action_pred = self.sparse_normalizer['action'].unnormalize(sparse_naction_pred)
        ...
        result = {
            'sparse': sparse_action_pred,
            'dense': dense_action_pred
        }
        return result
    ...
    def compute_loss(self, batch):
        ...
        nactions_sparse = self.sparse_normalizer['action'].normalize(batch['action']['sparse'])
        nactions_dense = self.dense_normalizer['action'].normalize(batch['action']['dense'])
        ...
        timesteps = torch.randint(
            ...
            (nactions_sparse.shape[0],), device=trajectory.device
        ).long()
        ...
        dense_naction_pred = self.predict_dense_action(
            ...
            sparse_ntraj = pred_trajectory.detach(), # (B, T, D)
        ...
        sparse_loss = F.mse_loss(pred_trajectory, target, reduction='none')
